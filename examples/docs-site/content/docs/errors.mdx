...
description: "Every Graft error code, what it means, and how to recover. 42 codes."
section: Reference
order: 4
---
...
`graft init` was run in a directory that already has a graft.config.
...
A branch with that name is already registered.
...
An operation referenced a branch that is not registered in the topology.
...
Content referenced a collection that is not registered in the schema.
...
git refused to record the commit. The working tree is untouched by the refusal.
...
No graft.config.\{ts,js} was found in the working directory or any parent.
...
The configured content directory does not exist on disk.
...
A document references another document that does not exist.
...
No document with the requested slug exists in that collection's files.
...
The invoked function name is not registered in the functions runtime.
...
A function invocation's input does not satisfy the function's Zod input schema.
...
A slug is not URL-safe kebab-case.
...
The endpoint was called with an HTTP method it does not serve.
...
The schema and the stored content/data have drifted; a migration must run first.
...
The capability is planned but not built yet.

**Usually because**

- Calling a placeholder API from a later phase
...
`graft add` was asked for a registry item (a copy-in primitive) that does not exist.
...
A document's frontmatter does not satisfy its collection's Zod schema.
...
Two documents in the same collection resolve to the same slug.
...
- Node older than 22.16 running the CLI or the app server
...
The caller's token does not permit this operation.
