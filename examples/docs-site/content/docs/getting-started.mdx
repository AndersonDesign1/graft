...
<Callout label="these docs describe the 1.0 beta">
  Install `@beta`. Everything here is written against `1.0.0-beta.x`, and the
  `latest` tag is still `0.2.0` — different enough that following this page on
  `latest` will not work. `approvalPolicy`, for one, moved from an environment
  variable into `graft.config.ts`.

  ```sh
  npx @usegraft/cli@beta init my-site
  ```

  `@beta` is a channel, not a snapshot: subscribe once and each new beta arrives
  as an ordinary upgrade. When 1.0 ships, drop the tag.
</Callout>
