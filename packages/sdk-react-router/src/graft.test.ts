 ...
 * through the structural loader/action args shape).
 */
...
describe("graftRoute", () => {
  it("hands the args' request to the Graft handler and returns its response", async () => {
  ...
  it("takes the whole args object React Router passes, not just a request", async () => {
    // The mount is typed structurally, so LoaderFunctionArgs — which carries
    // params and context as well — has to satisfy it without a cast. Built as
    // a typed value rather than inline: a fresh object literal would be
    // rejected for the extra properties and prove nothing about the real call.
    ...
    const args: LoaderArgs = {
      request: new Request("http://site.test/api/fn/pageStats"),
      params: { name: "pageStats" },
      context: {},
    };

    const route = graftRoute(async (request) => new Response(request.method));

    expect(await (await route(args)).text()).toBe("GET");
  });
});
