 ...
 * modules end up on disk), then appended llms.txt fragments.
 *
...
export function planAdd(names: readonly string[], options: PlanAddOptions): AddPlan {
  ...
  for (const item of items) {
    ...
    for (const file of item.files) {
      ...
      if (file.role === "module") newModules.push(basename(file.target).replace(/\.ts$/, ""));
    }
...
export function applyPlan(plan: AddPlan, options: { overwrite?: boolean } = {}): ApplyResult {
  ...
  return { written, skipped, barrelPath: plan.barrel.relPath, llmsAppended };
}
